...
| Base commit | Trial branch `trial/uig-mvp-1` at `8760dca` ("Trial setup"), on top of `4ec3baa` from `task/table-domain-and-field-ledger`. Local only, not pushed |
| CLI | `npx uig` resolves to a shim in the worktree's gitignored `node_modules/.bin`, which runs `<uigates>/bin/uig.mjs`. State lands in `.uig/` there |
...
1. **`npx uig help` can run someone else's package.** The skill tells agents to detect the CLI with `npx uig help`. Where the CLI is not installed, npx resolves `uig` from the npm registry, and an unrelated package of that name exists (version 0.0.0, modified 2022; not run). Whether npx installs it without asking depends on whether the shell is interactive. The trial is safe because the worktree has a local shim and the check uses `--no-install`. The five skill copies still say `npx uig help`; they should say `npx --no-install uig help`. Not changed yet, because the skills are the thing under test and `skills_sync_test` ties the copies together. Do it as its own change.
...
4. ~~Copy the skill into the trial worktree and commit it as "trial setup".~~ Done: `8760dca`.
