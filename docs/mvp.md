...
| FAIL | Evidence is missing or no longer matches its recorded hash; a receipt contradicts its own log; the verification command cannot fail (`true`, `echo`, `process.exit(0)`, or `git diff`/`show`/`log` without `--exit-code`, which exits 0 whether or not there is a difference) |
