...
- **The `uigates` CLI** (`../cli/main.ts`, launched by `bin/uigates.mjs`) is how a `/uigates` session records intents, proposals, authorizations, receipts and synthesis through this engine. Each call is a fresh process; `Runtime.ts` rebuilds the authority ledger, receipt store and cumulative risk from the write-once records in `.uigates/`. The skills tell the agent to use it when it is installed and to say so when it is not. `cli/cli_test.ts` drives it as separate processes.
...
- **The ledger is rebuilt from plain files.** `Runtime` reconstructs it on every CLI call, so cross-process rules hold, but nothing signs those files: a hostile agent with write access to `.uigates/` can forge records. Signed records with a key the agent cannot read would close this; not built. `cli_test` covers a forged authorization file that the engine's own checks refuse, not a forger who is consistent throughout.
...
- **The write-time hook sees only the file-editing tools.** `uigates hook pre-write` (a Claude Code PreToolUse hook, shipped in `hooks/hooks.json`) refuses a Write, Edit, MultiEdit or NotebookEdit that no unspent authorization covers. A write made through Bash (`cat >> file`, `sed -i`) does not pass through it, and neither does anything an agent does outside the harness; the audit remains the backstop for those. In the first trial, the two files written before they were authorized both went through Write and Edit, and the CI edit that followed went through Bash. It is off by default (`uigates enforce on`, or `UIGATES_ENFORCE=1`), because an intent lasts 24 hours and a hook that locked a project for that long during ordinary work would be worse than none. It fails open: only a definite refusal exits 2, which is what blocks the tool call, and any internal error or unreadable record exits 1. It adds a process start to every edit.
