...
try {
  if(!command||command==='help'){
    ...
    switch(command){
      case'discover':{const spec=input();if(spec.mode!=='discovery')throw Error('Expected discovery mode');result=await runTask(store,spec,{codex:process.env.UIG_CODEX});break;}
      case'propose':result=memory.propose(input());break;
      case'freeze':result=memory.freeze(input());break;
      case'run':await runPlan(store,arg,{codex:process.env.UIG_CODEX,progress:r=>console.log(JSON.stringify(r))});break;
